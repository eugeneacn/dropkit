...
```bash
# Kit-installer install — exposes an `ai-adoption-report` shim on PATH:
...
From a dropkit clone (before kit-installer ships), add the package to
PYTHONPATH:
...
`python -m ai_adoption_report` if you're invoking from a clone.
