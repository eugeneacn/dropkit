...
def _find_skill_root(start: Optional[Path] = None) -> Path:
    ...
    Layout-agnostic so the same resolver works for kit-installer installs
    (``~/.claude/skills/flow-metrics/``) and dropkit clones
    (``<repo>/skills/workflows/flow-metrics/``). During development the
