...
- **`jira` skill** (sibling in dropkit) — every Jira read: `check`,
...
- **`jira-align` skill** (sibling in dropkit) — used **only** for
...
1. The `jira` skill is installed and authenticated. Invoke it:
   `jira: check`. Exit 0 → proceed. Exit 2 → tell the user to run the
   jira skill's `setup_credentials.sh` themselves; do not attempt to
   read `~/.config/dropkit/credentials.env` from this skill.
